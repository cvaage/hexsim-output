# HexSim Results Processing: Single Species Model
...
input_folder <- "data/smb"
output_folder <- "output/1_smb/1.3_smb_density_ss"

# List files from input folder
file_list <- list.files(path = input_folder, pattern = "\\.csv$", recursive = TRUE, full.names = TRUE)
file_list <- file_list[grepl("Density", file_list)] #filter to 10 replicates
...
for(file in file_list){
  ...
  bass_dens <- df_long %>% 
    ...
    select(Step, RID, Density_Adults) #remove old column
    
  new_file_name <- paste0("/smb_density_ss_", counter, ".csv")
