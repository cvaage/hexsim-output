...
output_folder <- "output/1_smb/1.1_smb_occupancy_ss"

# List files from input folder
file_list <- list.files(path = input_folder, pattern = "\\.csv$", recursive = TRUE, full.names = TRUE)
file_list <- file_list[grepl("monthly", file_list)] #filter to 10 replicates
...
for(file in file_list){
  ...
  new_file_name <- paste0("/smb_occupancy_ss_", counter, ".csv")
